      ...
      <FadeIn>
        ...
        <div className='max-w-xs mx-auto mb-12'>
          ...
          <div className='grid grid-cols-7 mb-4 text-center'>
            {weekDays.map((day, i) => (
              <div
                key={day}
                className={`text-[14px] font-myeongjo ${i === 0 ? 'text-red-400' : 'text-charcoal/70'}`}
              >
                {day}
              </div>
            ))}
            ...
            {days.map((day) => {
              ...
              return (
                <div key={day} className='relative flex items-center justify-center'>
                  <span
                    className={`
                       relative z-10 w-8 h-8 flex items-center justify-center text-[1rem] font-serif
        ...
        <div className='text-center border-y border-gold/20 py-8 max-w-sm mx-auto flex flex-col items-center'>
          ...
          <p className='text-charcoal font-myeongjo'>
            예식일까지{' '}
            <span className='text-gold font-bold text-xl mx-1 font-serif'>{daysLeft}</span>일
            남았습니다
          </p>
